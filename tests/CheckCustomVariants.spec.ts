...
describe('CheckCustomVariants', () => {
    ...
    for(const figure of ['N', 'B', 'R', 'Q']) {
        it(`check pawn promotion ${figure}`, () => checkGame(chess, deployer.getSender(), toGame(`1. e4 d5 2. exd5 Nf6 3. Bb5+ c6 4. dxc6 Qb6 5. cxb7+ Qxb5 6. bxc8=${figure}`)));
    }
    it('check king under attack', async ()=>{
        await checkGame(chess, deployer.getSender(), toGame(`1. e4 d5 2. exd5 Nf6 3. Bb5+ c6 4. dxc6 Qb6 5. cxb7+ Qxb5 6. bxc8=R`));
        // try to move invalid
        const {transactions: tr} = await chess.send(deployer.getSender(), {
            value: toNano(1),
        }, {
            $$type: 'MoveMsg',
            move: {
        ...
        expectSuccessMove(chess, await chess.send(deployer.getSender(), {value: toNano(1)}, {
            $$type: 'MoveMsg',
            promotion: 0n,
            move: {
                fromX: 4n,
